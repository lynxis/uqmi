...
int
tx_dms_set_operating_mode(struct modem *modem, struct qmi_service *dms, uint8_t operating_mode, request_cb cb)
{
	struct qmi_request *req = talloc_zero(dms, struct qmi_request);
	struct qmi_msg *msg = talloc_zero_size(req, 1024);

	struct qmi_dms_set_operating_mode_request set_op_mode_req = {
	    QMI_INIT(mode, operating_mode)
	};
...
int
tx_nas_subscribe_nas_events(struct modem *modem, struct qmi_service *nas, bool action, request_cb cb)
{
...
int
tx_wds_get_profile_list(struct modem *modem, struct qmi_service *wds, request_cb cb) {
	struct qmi_request *req = talloc_zero(wds, struct qmi_request);
	struct qmi_msg *msg = talloc_zero_size(req, 1024);

	struct qmi_wds_get_profile_list_request profile_req = {
...
int tx_wds_start_network(struct modem *modem, struct qmi_service *wds, request_cb cb, uint8_t profile_idx, uint8_t ip_family) {
	struct qmi_request *req = talloc_zero(wds, struct qmi_request);
	struct qmi_msg *msg = talloc_zero_size(req, 1024);
...
int tx_wds_stop_network(struct modem *modem, struct qmi_service *wds, request_cb cb,
			uint32_t packet_data_handle, bool *disable_autoconnect) {
	struct qmi_request *req = talloc_zero(wds, struct qmi_request);
	struct qmi_msg *msg = talloc_zero_size(req, 1024);
...
int tx_uim_read_transparent_file(struct modem *modem, struct qmi_service *uim, request_cb cb,
