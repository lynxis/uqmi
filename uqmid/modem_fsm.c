...
static void
ctl_sync_request_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
...
static void
uqmid_modem_version_cb(struct qmi_service *ctl_service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
...
static void
get_manuf_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	int ret = 0;
...
static void
get_model_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	int ret = 0;
...
static void
get_revision_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	int ret = 0;
...
static void
uim_get_slot_status_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	int ret = 0, i = 0;
	char *iccid_str;
	...
	if (req->ret) {
		modem_log(modem, LOGL_INFO, "Failed to get slot status. Returned %d/%s. Trying next card status", req->ret, qmi_get_error_str(req->ret));
	...
	for (i = 0; i < res.data.physical_slot_status_n; ++i) {
		...
		ret = osmo_bcd2str(
			iccid_str, iccid_str_len,
			iccid_bcd, 0, iccid_n, 1);
...
static void
uim_get_card_status_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	...
	for (i = 0; i < res.data.card_status.cards_n; ++i) {
		...
		for (int j = 0; j < res.data.card_status.cards[i].applications_n; ++j) {
			...
			modem_log(modem, LOGL_INFO, "Found valid card application type %x", res.data.card_status.cards[i].applications[j].type);
			...
			if (modem->sim.use_upin) {
				...
				modem->sim.state = uim_pin_to_uqmi_state(res.data.card_status.cards[i].applications[i].pin1_state);
...
static void modem_st_get_imsi(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MODEM_EV_RX_UIM_GET_SLOT_FAILED:
		if (uim)
			uqmi_service_send_simple(uim, qmi_set_uim_get_card_status_request, uim_get_card_status_cb, modem);
		else
			osmo_fsm_inst_dispatch(modem->fi, MODEM_EV_RX_UIM_FAILED, NULL);
		break;
...
static void
dms_get_operating_mode_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	int ret = 0;
...
static void
dms_set_operating_mode_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	if (req->ret) {
		modem_log(modem, LOGL_INFO, "Failed to set operating mode. Status %d/%s.", req->ret, qmi_get_error_str(req->ret));
		osmo_fsm_inst_dispatch(modem->fi, MODEM_EV_RX_FAILED, NULL);
		return;
	}
...
static void
wds_modify_profile_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	...
	int ret;
	if (req->ret) {
		modem_log(modem, LOGL_INFO, "Failed to modify profile list. Status %d/%s.", req->ret, qmi_get_error_str(req->ret));
...
static void
wds_get_profile_list_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	...
	int ret;
	if (req->ret) {
		modem_log(modem, LOGL_INFO, "Failed to get profile list. Status %d/%s.", req->ret, qmi_get_error_str(req->ret));
		osmo_fsm_inst_dispatch(modem->fi, MODEM_EV_RX_FAILED, NULL);
		return;
	}
...
static void modem_st_configure_modem(struct osmo_fsm_inst *fi, uint32_t event, void *data)
{
	...
	case MODEM_EV_RX_GET_PROFILE_LIST:
		...
		if (data) {
			...
			tx_wds_modify_profile(modem, wds, wds_modify_profile_cb,
					      modem->qmi->wds.profile_id,
					      modem->config.apn, modem->config.pdp_type,
					      modem->config.username, modem->config.password);
		}
		break;
...
static void
subscribe_result_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	if (req->ret) {
		modem_log(modem, LOGL_INFO, "Service %d: Subscribe check failed with %d/%s", service->service, req->ret, qmi_get_error_str(req->ret));
		osmo_fsm_inst_dispatch(modem->fi, MODEM_EV_RX_SUBSCRIBE_FAILED, (void *) (long) le32_to_cpu(msg->svc.message));
		return;
	}
...
static void
get_serving_system_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	struct qmi_nas_get_serving_system_response res = {};

	if (req->ret) {
		modem_log(modem, LOGL_INFO, "Failed to get system info. Status %d/%s", req->ret, qmi_get_error_str(req->ret));
...
static void
nas_force_search_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	if (req->ret) {
		modem_log(modem, LOGL_INFO, "Failed to force a network search. Status %d/%s", req->ret, qmi_get_error_str(req->ret));
		/* FIXME: charge timer */
		osmo_fsm_inst_dispatch(modem->fi, MODEM_EV_RX_FAILED, NULL);
		return;
	}
...
static void
wds_start_network_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	...
	if (ret && ret == QMI_PROTOCOL_ERROR_NO_EFFECT) {
		...
		modem_log(modem, LOGL_INFO, "Failed start network. QMI Status %d/%s.", req->ret, qmi_get_error_str(req->ret));
...
static void
ipv4_to_sockaddr(const uint32_t wds_ip, struct sockaddr_in *addr)
{
...
static void
ipv6_to_sockaddr(const uint16_t *wds_ip6, struct sockaddr_in6 *addr)
{
	memset(addr, 0, sizeof(*addr));
...
static void
wds_get_current_settings_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
	struct modem *modem = req->cb_data;
	...
	case QMI_WDS_IP_FAMILY_IPV4:
		...
		if (res.set.ipv4_gateway_subnet_mask) {
			ipv4_to_sockaddr(res.data.ipv4_gateway_subnet_mask, (struct sockaddr_in *) &modem->brearer.v4_netmask);
		}

		if (res.set.ipv4_gateway_address) {
			ipv4_to_sockaddr(res.data.ipv4_gateway_address, (struct sockaddr_in *) &modem->brearer.v4_gateway);
		}
		...
		if (res.set.secondary_ipv4_dns_address) {
			ipv4_to_sockaddr(res.data.secondary_ipv4_dns_address, (struct sockaddr_in *) &modem->brearer.dns2);
		}
		break;
	case QMI_WDS_IP_FAMILY_IPV6:
		...
		if (res.set.ipv6_primary_dns_address) {
			ipv6_to_sockaddr(res.data.ipv6_primary_dns_address, (struct sockaddr_in6 *) &modem->brearer.dns1);
		}

		if (res.set.ipv6_secondary_dns_address) {
			ipv6_to_sockaddr(res.data.ipv6_secondary_dns_address, (struct sockaddr_in6 *) &modem->brearer.dns2);
		}
...
static int modem_fsm_timer_cb(struct osmo_fsm_inst *fi)
{
	...
	case MODEM_ST_START_IFACE:
		...
		case N_RESEND:
			...
			tx_wds_start_network(modem, service, wds_start_network_cb, modem->qmi->wds.profile_id, modem->qmi->wds.ip_family);
			osmo_timer_schedule(&fi->timer, 5, 0);
			break;
		default:
