					 ...
					 enum uim_file_names file_name)
{
    ...
    return tx_uim_read_transparent_file(modem, uim, cb,
					file->file_id,
					&path[0], file->path_n*2);
}
...
static void uim_read_imsi_cb(struct qmi_service *service, struct qmi_request *req, struct qmi_msg *msg)
{
    ...
    if (req->ret) {
	modem_log(modem, LOGL_INFO, "Failed to read imsi. Qmi Ret %d/%s.", req->ret, qmi_get_error_str(req->ret));
...
static void sim_fsm_allstate_action(struct osmo_fsm_inst *fi, uint32_t event,
				    void *data) {
    switch (event) {
...
struct osmo_fsm_inst *sim_fsm_alloc(struct modem *modem) {
    struct osmo_fsm_inst *fi = osmo_fsm_inst_alloc_child(&sim_fsm,
				     modem->fi,
